...
Flutter 或插件维护方声明 Android 支持，也不代表依赖 Google Play 服务的功能能在中国大陆常见设备环境中工作。本课程选择插件和云服务时，会基于官方一手资料分开核对：
...
因此，后续的“Android 支持”分为两层：平台或插件维护方的一手资料声明，以及课程在 Android 模拟器或构建链上的实际结果。当前 `Spark` 已完成 Android 16 模拟器运行、基础交互、热重载、debug APK 和使用模板调试密钥的 release AAB 构建；这仍不能替代无 GMS 大陆真机、具体插件、发布签名和应用商店流程的逐项验证。“三端支持”也不会被笼统写成三端能力完全等价。
